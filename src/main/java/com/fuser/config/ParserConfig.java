     ...
     public static void configureParser(Parser parser) {
        ...
        parser.register(new Parselet(SELECT) {
            ...
            public Expression parse() {
                ...
                if(canConsume(word("HAVING"))){
                    Expression having = new Expression("HAVING", lastMatch());
        ...
        //CAST
        parser.register(new Parselet(CAST) {
            @Override
            public boolean isPrefixParselet() {
                return true;
            }
            ...
            @Override
            protected Expression parse() {
                Expression cast = new Expression("FUNCTION", lastMatch());
                Expression child = nextExpression();
                consumeIf("\\)");
                cast.addChildren(Expression.e(child.printValue(), "ALIAS-CAST", child.getChildren()));
                return cast;
            }
        });
