...
public class FuserTest {
    ...
    public void testLimitUnion(){
        ...
        assertEquals("ORACLE", "SELECT * FROM (SELECT id1  id FROM Tb WHERE campo = 3 UNION SELECT campo2  id FROM tb2 ORDER BY id) WHERE ROWNUM <= 50", fuser.toOracle());
    ...
    public void testCast(){
        ...
        assertEquals("MYSQL", "SELECT CAST(campo AS SIGNED) FROM TAB1", fuser.toMySQL());
    ...
    @Test
    public void tesAliasOracle(){
        Fuser fuser = new Fuser("SELECT * FROM TAB1 AS TABLE");
        assertEquals("POSTGRES", "SELECT * FROM TAB1 AS TABLE", fuser.toPostgreSQL());
        assertEquals("FIREBIRD", "SELECT * FROM TAB1 AS TABLE", fuser.toFirebird());
        assertEquals("MYSQL", "SELECT * FROM TAB1 AS TABLE", fuser.toMySQL());
        assertEquals("ORACLE", "SELECT * FROM TAB1  TABLE", fuser.toOracle());
        assertEquals("SQLSERVER", "SELECT * FROM TAB1 AS TABLE", fuser.toSQLServer());
    }

    @Test
    public void tesAliasComCastOracle(){
        Fuser fuser = new Fuser("SELECT CAST(campo AS VARCHAR) FROM TAB1 AS TABLE");
        assertEquals("POSTGRES", "SELECT CAST(campo AS VARCHAR) FROM TAB1 AS TABLE", fuser.toPostgreSQL());
        assertEquals("FIREBIRD", "SELECT CAST(campo AS VARCHAR) FROM TAB1 AS TABLE", fuser.toFirebird());
        assertEquals("MYSQL", "SELECT CAST(campo AS CHAR) FROM TAB1 AS TABLE", fuser.toMySQL());
        assertEquals("ORACLE", "SELECT CAST(campo AS VARCHAR2) FROM TAB1  TABLE", fuser.toOracle());
        assertEquals("SQLSERVER", "SELECT CAST(campo AS VARCHAR) FROM TAB1 AS TABLE", fuser.toSQLServer());
    }

    @Test
    public void testNull(){
        Fuser fuser = new Fuser("SELECT * FROM TABELA WHERE CAMPO = ''");
        assertEquals("ORACLE", "SELECT * FROM TABELA WHERE CAMPO IS NULL", fuser.toOracle());
        assertEquals("POSTGRES", "SELECT * FROM TABELA WHERE CAMPO = ''", fuser.toPostgreSQL());

        fuser = new Fuser("SELECT * FROM TABELA WHERE CAMPO = NULL");
        assertEquals("ORACLE", "SELECT * FROM TABELA WHERE CAMPO IS NULL", fuser.toOracle());
        assertEquals("POSTGRES", "SELECT * FROM TABELA WHERE CAMPO = NULL", fuser.toPostgreSQL());

        fuser = new Fuser("SELECT * FROM TABELA WHERE CAMPO != ''");
        assertEquals("ORACLE", "SELECT * FROM TABELA WHERE CAMPO IS NOT NULL", fuser.toOracle());
        assertEquals("POSTGRES", "SELECT * FROM TABELA WHERE CAMPO != ''", fuser.toPostgreSQL());
        ...
        fuser = new Fuser("SELECT * FROM TABELA WHERE CAMPO = id");
        assertEquals("ORACLE", "SELECT * FROM TABELA WHERE CAMPO = id", fuser.toOracle());
        assertEquals("POSTGRES", "SELECT * FROM TABELA WHERE CAMPO = id", fuser.toPostgreSQL());
